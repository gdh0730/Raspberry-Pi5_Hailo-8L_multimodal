#!/usr/bin/env python3
"""
Analyze phase-3.5 next-direction v6 FP32 runs.
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Analyze phase35 next v6 runs")
...
def main() -> None:
    ...
    specs = [
        ("fp32_v6_ce_ls_ws_main", Path("derived/results/fp32_multitask_phase35_v6_ce_ls_ws_main/summary.json"), "fp32"),
        ("fp32_v6_focal_ws_main", Path("derived/results/fp32_multitask_phase35_v6_focal_ws_main/summary.json"), "fp32"),
        ...
        rows.append(
            {
                ...
                "delta_f1_vs_phase2_main": float(m["emotion_macro_f1"]) - float(refs["phase2_main_fusion"]["emotion_macro_f1"]),
                "delta_f1_vs_phase3_main": float(m["emotion_macro_f1"]) - float(refs["phase3_main_fp32"]["emotion_macro_f1"]),
                "delta_f1_vs_v5_logreg_main": float(m["emotion_macro_f1"]) - float(refs["phase35_v5_logreg_main"]["emotion_macro_f1"]),
            }
        )

    rows.sort(key=lambda x: float(x["emotion_macro_f1"]), reverse=True)

    out_csv = args.out_dir / "phase35_next_v6_metrics.csv"
    ...
    lines.append("- `derived/reports/phase35_next_v6_metrics.csv`")
    lines.append("- `derived/reports/phase35_next_v6_results.md`")

    out_md = args.out_dir / "phase35_next_v6_results.md"
    out_md.write_text("\n".join(lines) + "\n", encoding="utf-8")
